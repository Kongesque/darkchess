...
/*
 * Generate the best move of current player using alpha-beta pruning
 */
MOVE MyAI::GenerateMove() const {

    // Create the root node for the alpha-beta search
    Node rootNode = createNode(board, numeric_limits<int>::min(), numeric_limits<int>::max(), 0);
    
    // Call alpha-beta with root node
    ...
    for (MOVE move : legalMoves) {
        ...
        if (from == to) { // Flip move
            FIN flippedPiece = (FIN)(rand() % FIN_COVER); // Random flip
            if (tempCoverPieceCount[flippedPiece] == 0) { // Handle no left cover pieces
...
/*
 * Get all legal moves of current player
 */
